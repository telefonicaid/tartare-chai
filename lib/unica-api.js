...
var requireLazy = codependency.register(module.parent, {index: ['lazyDependencies']});
...
module.exports = function(chai, utils) {
  ...
  Assertion.addProperty('wellFormedSoap11ApiResponse', function assertWellFormedSoap11ApiResponse() {
    ...
    if (isXml) {
      var isSoap11 = xmlDoc.validate(schemas.soap11.get());
      this.assert(
          isSoap11 === true,
  ...
  Assertion.addMethod('jsonApiError', function assertJsonApiError(
    ...
    var jsonValidate = jsonValidator(schemas.unicaApiErrors.json[expectedVersion].get(), {verbose: true});
    this.assert(
  ...
  Assertion.addMethod('xmlApiError', function assertXmlApiError(
    ...
    this.assert(
        xmlBody.validate(schemas.unicaApiErrors.xml[expectedVersion].get()),
  ...
  Assertion.addMethod('soap11ApiError', function assertSoap11ApiError(
    ...
    this.assert(
        soapBody.validate(schemas.unicaApiErrors.soap11[expectedVersion].get()),
        'expected HTTP Body to validate against the UNICA API schema',
        'expected HTTP Body to not validate against the UNICA API schema',
        undefined,
        res.body
