...
module.exports = function(chai, utils) {
  ...
  Assertion.addMethod('httpHeaders', function assertHttpHeaders(expectedHeaders, valuesCaseInsensitive) {
    ...
    if (Array.isArray(expectedHeaders)) {
      ...
      // Create an object with array's elements as keys and undefined as values
      expectedHeaders = _.zipObject(expectedHeaders, []);
    }
  ...
  Assertion.addMethod('httpAllowHeader', function assertHttpAllowHeader(expectedAllowHeaderValue) {
    var actualHeaders = this._obj.headers;
    var lowerCasedActualHeaders = tartareHttp.lowerCaseHeaders(actualHeaders);
    var negated = utils.flag(this, 'negate');

    this.assert(
        lowerCasedActualHeaders.hasOwnProperty('allow'),
    ...
    if (expectedAllowHeaderValue) {
      var actualAllowHeaderValue = lowerCasedActualHeaders['allow'];  // eslint-disable-line dot-notation
      var sortedActualAllowHeader = actualAllowHeaderValue.trim().split(/\s*,\s*/).sort();
      var sortedExpectedAllowHeader = expectedAllowHeaderValue.sort();
      this.assert(
  ...
  Assertion.addMethod('httpQueryParams', function assertHttpQueryParams(expectedQueryParams) {
    var actualQueryParams = this._obj.requestUri.indexOf('?') !== -1 ?  // eslint-disable-line no-negated-condition
